...
public class ListRun {
	...
	/*
	 * 컬렉션(Collection)
	 * - 자료구조가 내장되어 있는 클래스로, 자바에서 제공하는 자료구조를 담당하는 프레임워크
	 * 
	 * 자료구조
	 ...
	 *  - 효율적인 기능들이 정의되어 있는 틀
	 *  
	 * 데이터들이 새롭게 추가되거나 삭제가 되거나, 수정이 되는 기능들을 이미 정의해놓은 틀이다.
	 * 
	 * 배열 컬렉션(List)의 차이점
	 * 	- 배열의 단점
	 ...
	 *  - 컬렉션의 장점
	 *  
	 *  1. 여러자료형의 값을 저장할 수 있다.
	 *   -> Object 객체 때문
	 *   -> 제네릭 설정을 통해 한가지 유형의 데이터만 저장도 가능
	 * 	2. 중간에 값을 추가하거나 삭제하는 등 배열내부의 값들을 조작하기 위한 편리한
	 * 	메서드가 정의되어 있다.
	 * 	3. 크기에 대한 제약이 없다.
	 *  -> 배열을 더 추가하면 자동으로 깊은 복사를 해주기 때문
	 *  -> 기존 배열은 삭제
	 * 
	 * 
	 * 컬렉션의 3가지 분류
	...
	public static void main(String[] args) {
		...
		//크기 10 배열 생성
		//ArrayList는 List 클래스의 자식객체이다
		
	  
		/*
		 * 제네릭 ( <E> )
		 *  - 별도의 제네릭 설정없이 컬렉션 생성이 제네릭은 Object로 설정된다.
		 *  - 제네릭을 설정하면 설정한 타입의 값들만 컬렉션에 담을 수 있다.
		 *  
		 * 제네릭을 설정하는 이유
		...
		// 초기화가 완료된 갯수까지만 배열값을 보여준다.
		
		
		//1 . add(E e) : 리스트의 끝(초기화된 위치 기준)에 전달된 e를
		//    추가시켜주는 메서드
		...
		//2.add(int index, E e)
		//	- 리스트에 전달되는 index값 위치에 e를 '추가'하는 메서드
		
		list.add(0, new Music("바다의 왕자", "박명수"));
		// 중간에 값 추가시 알아서 기존값들을 뒤로 한칸식 땡겨주는 깊은 복사가 내부적으로 발생한다.
		
		System.out.println(list);
		
		//3. set(int index , E e)
		//  - 리스트에 해당 인덱스의 값을 전달되는 e로 '변경'시켜주는 메서드
		...
		//만약 제네릭 설정을 안했다면?
		Music m2 = (Music) list.get(0); // 강제 형변환이 필요하다
		
		
		ArrayList<Integer> Iarr = new ArrayList<>(3);
		// 제네릭에 자료형은 불가능 해서 wrapping 해줘야 함
		...
		//7. contains(E e) -> boolean 값 반환
		System.out.println(Iarr.contains(9));
		
		//8. indexOF(E e) 
		// -매개 인자 요소가 몇번 인덱스에 있는지 그 값을 반환(없다면 -1)
		...
		// Iterator 에 각 배열의 값을 하나하나 저장하고 next로 순서대로 꺼내는 방식
		// StringTokenizer와 비슷
