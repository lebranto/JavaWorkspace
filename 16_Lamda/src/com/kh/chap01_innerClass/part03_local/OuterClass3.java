...
public class OuterClass3 {
	 ...
	 * 	- 메서드 안에서만 사용 가능, 메서드가 호출될 떄 생성된다.
	 *  - 지역내부 클래스 안에서 사용되는 지역변수는 "상수(final)"처럼 취급 된다.
	 *   (메모리 효율성을 위해)
	 *   
	 *  메모리 복습
	 *  1) stack
	 *   - 메서드 호출시 스택에는 메서드프레임이 쌓인다.
	 *   - 메서드 프레임 내부에는 지역변수를 위한 저장 공간이 할당된다.
	...
	public Runnable getRunnable(int num2) {
		...
		// 메모리가 stack 메모리에서 지역변수의 저장공간이 소멸된 후 참조를 유지하기 위해 
		// static 메모리의 상수풀로 욺기기 때문 
		...
		class LocalInnerClass implements Runnable{
			...
			public void run() {
				...
				// 지역변수는 메서드 종료시 스택 공간에서 함께 소멸된다.
				// 근데 heap 메모리에 저장되어 있는 내부 클래스 안에서 사용 할 수 있다. 
