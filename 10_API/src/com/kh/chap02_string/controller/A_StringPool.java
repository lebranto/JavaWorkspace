...
public class A_StringPool {
	 ...
	 * String은 기존에 담겨 있던 값이 수정되는 순간 원래 담겨 있던 공간에서 수정되지 않고,
	 * 새로운 저장공간을 할당한 후, 거기에 값을 추가 한다.
	 * 
	 * 
	 */
	
	//	문자열 상수 풀(String constant pool)
	// - 문자열 데이터가 담기는 영역으로, heap메모리 영역상에 항상 고정적으로 할당 되어 있다.
	
	public void method1() {
		String str1 = new String("hello");
		String str2 = new String("hello");
		
		System.out.println(str1 == str2); //false(주소값 비교기 때문)
		System.out.println(str1); 
		// hello 가 나오는 이유는 toString() 메서드가 오버 라이딩 되어 있기 때문
		System.out.println(str2);
		
		System.out.println(str1.equals(str2)); // true(문자열[값] 비교기 때문)
		
		System.out.println(str1.hashCode()); // 주소값 반환 메서드
		System.out.println(str2.hashCode()); // 주소값 반환 메서드
		// 같게 나오는데 이는 오버라이딩으로 인해 똑같은 문자열을 같은 값으로 하기 때문
	...
	public void method2() {
		 ...
		 * 
		 * 문자열 상수풀 동작방식(jvm)
		 * 1. 프로그램 실행 도웆 문자열 리터럴을 만나면, 우선 문자열 리터럴 값이 이미
		 ...
		 * 	- 문자열은 문자열 기반 hashCode값을 상수풀상의 "주소값"으로 사용하고 있다.
		 * 	  따라서 상수풀 안에 데이터를 저장할 때 문자열을 해시코드값으로 변환 한 후 그 자리에
		 * 	  데이터가 있는지 없는지 검사를 길행하고 이미 값이 있다면 해당 주소값을 변수에 할당
