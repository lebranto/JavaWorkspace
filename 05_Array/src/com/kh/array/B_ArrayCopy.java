...
public class B_ArrayCopy {
	/*
	 * 배열복사
	 * - 얕은 복사 : 배열의 주소값만을 복사. 내부값은 복사 안됨
	 * - 깊은 복사 : 동일한 새로운 배열을 하나 생성하여 내부의 값들까지 복사하는 개념
	 * 
	 * 	얕은 복사를 하면 heap 메모리의 변화가 없고 깊은 복사를 하면 변화가 있다.
	 * 	얕은 복사를 하면 주소만 복사하기 때문에 주소 값이 변경되면 얕은 복사끼리 공유된다.
	...
	public void method2() {
		//1. for 문을 활용한 복사
		// - 새로운 배열을 생성한 후 각 인덱스 별로 내부값을 일일히 대입
		int [] origin = {1,2,3,4,5};
