package com.kh.chap02_.protocol.tcp.server.run;
...
public class Run {
	public static void main(String[] args) {
		 ...
		 *   - 서버의 ip주소, 포트번호를 통해 서버에게 연결 요청을 보낸다.
		 */
		
		// 1) 서버 소켓의 포트번호 지정
		// 		- 0 ~ 1023 : 시스템 포트번호로 사용 불가
		//      - 1024 ~ 35535 사이의 포트 번호 사용
		
		int port = 30000;
		
		// 2) 서버 소켓 객체 생성
		//  - 현재 pc의 ip주소와 port 번호를 통해 소켓 생성
		...
		try {
			...
			//3)클라이언트의 접속 요청이 들어오기 까지 대기.
			while(true) {
				System.out.println("### 클라이언트의 요청 대기중 ###");
				// 클라이언트의 요청이 들어오기 전까지 대기 후, 요청이 들어오면
				// 클라이언트와 통신이 가능한 Socket을 생성
				Socket client = server.accept();
				System.out.println("### 클라이언트와 연겸됨 ! ###");
				System.out.println(""
						+ "클라이언트의 ip 주소 : "+
				  client.getInetAddress().getHostAddress());
				// 아이피를 inetAddress 객체로 가져오고 문자열로 한번더 가져오기
				
				// 4) 연결된 클라이언트와 통신 할 수 있는 입출력 스트림 생성.
