...
public class Run {
	public static void main(String[] args) {
		...
		/* 
		 * '=' (대입 연산사)를 기준으로 왼쪽과 오른쪽의 자료형은 항상 같아야 한다.
		 * 1. 부모타입 참조변수로 부모타입 객체를 다루는 경우 
		 * 
		 * */
		
		System.out.println("1. 부모타입 참조변수(레퍼런스)로 부모객체 다루기");
		...
		c1.printChild1(); // this(자식) 에서 호출
		
		
		//3.부모타입 참조변수로 자싯타입 객체를 다루는 방법
		System.out.println("3. 부모타입 참조변수로 자식타입 객체 다루기.");
		Parent p2 = new Child1(); //다형성이 적용된 개념
		//parent p2 = (parent) new child 와 똑같음
		
		p2.printParent(); //p2(부모타입변수)로는 parent 클래스 내부의 메서드 필드만 접근가능
		//자식클래스의 메서드에 접근하고자 한다면, DownCasting이 필요.
		((Child1)p2).printChild1(); // 다운 캐스팅

		/*
		 * 대입 연산자 기준 양쪽의 자료형이 다름에도 불구하고 오류가 나지 않는다.
		 * 이유는, 자식객체가 부모 자료형으로 '자동형변환'되고 있기 때문.
		 * 상속구조의 경우 클래스간의 형변환이 가능.
		 * 
		 * 클래스간 형변환
		 * 1. UpCasting 
		 * - 자식타입에서 부모타입으로 자동 형변환
		 * - 자동 형변환이 가능하므로 형변환 연산자는 생략이 가능
		 * - ex ) 자식객체.부모메서드();
		 * 		  부모참조변수 = 자식객체;
		 * 
		 * 2. DownCasting
		...
		arr2[1] = new Child2 (4,5);
		
		// 다형성이 적용된다면, 부모타입 레퍼런스로 다양한 자식객체들을 받아줄 수 있다.
		Parent[]  arr = new Parent [4];
		arr[0]= new Child1(1,2,3);
		arr[1]= new Child2(4,5);
		arr[2]= new Child1(6,7,8);
		arr[3]= new Child2(9,10);
		
		
		// 반복문을 활용한 메서드 호출
		for (int i = 0 ; i<arr.length; i++) {
			// 각 인덱스 별로 실제 참조하고 있는 자식클래스로 적절히 다운 캐스팅 후,
			// 메서드를 호출하려면?
