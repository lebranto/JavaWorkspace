...
public class FieldTest1 {
	 ...
	 * 
	 * 변수 구분 
	 * 	- 전역변수 : 클래스 영역 바로 아래에 선언하는 변수를 의미 클래스 내부라면 어디서든 사용 가능 
	 * 	- 지역변수 : 클래스 영역 내에서 특정영역({})에 선언한 변수를 의미. 선언한 영역에서만 사용이 가능(if, 메서드, for ....)
	 * 
	 * 1. 전역변수 (고정적으로 메모리 공간 차지) 
	 * 	- 필드 == 인스턴스 변수 == 멤버변수 
	 ...
	 * 소멸시점 :프로그램 종료시. ex) public static int age;
	 * 
	 * 
	 * 2. 지역변수 (일시적으로 메모리 공간 차지) 
