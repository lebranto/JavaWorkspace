...
public class B_StringBuilderAndBuffer {
	 ...
	 * 	- 두 클래스의 사용방법은 동일함
	 *  - 차이점은 "동기화" 여부 //정확한건 15 스레드에서 배움
	 *  - StringBuilder는 동기화를 지원하지 않기 때문에 "멀티스레드 환경"에서 좋은 성능을 보인다. 
	 *  - StringBuffer는 동기화를 지원하기 때문에 "멀티스레드 환경"에서 스레드 안전성을 보장할 수
	 *    있으나 성능이 좋지 못하다. 
	 * 
	 */
	...
	public void method () {
		...
		StringBuilder sb1 = new StringBuilder("hello"); //sb1 주소값에 저장
		sb1.append("World"); //같은 저장공간에서 이어쓰기 가능
		sb1.append("!!!");// 주소값 1개만 사용
		// append는 문자열을 추가한다는 함수
		
		StringBuffer sb2 = new StringBuffer();
		sb2.append("World"); // append에 초록색 아이콘 시계모양이 있는 건 동기화중이라는 뜻
		
		
		System.out.println(sb1);
		System.out.println(sb2);
		//참조 자료형을 문자열로 변경
		// toString 오버라이딩이 되어 있어 값으로 나온다.
