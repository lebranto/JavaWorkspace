package com.kh.chap03.stream.part03_methods;
...
public class OptionalMethods {
	...
	public static void main(String[] args) {
		 ...
		 * 3. Optional.ofNullable(obj) : null 일 수도 있는 객체 생성 
		 * 
		 * 
		 * Optional 객체 내부 메서드들
		 *  - get() : Optional 내부의 값을 반환하는 메서드.
		 *     -> 값이 없다면 NoSuchElement예외 발생)
		 *  - getAs자료형() : 해당 자료형의 값을 반환하는 메서드
		 *  - orElse(T) : get()으로 꺼낼 값이 존재하지 않을 경우 반환할 값.   
		 *  - orElseGet(Supplier) : get()으로 꺼낼 값이 존재하지 않을 경우
		 *    람다식을 통해 값을 반환, Supplier이니 매개변수x 반환값o
		 *  - orElseThrow(Supplier) : get()으로 꺼낼 값이 없을 떄, 람다식을 통해 구현한
		 *    에러 객체를 호출  
		 */
...
//		//optional 없이 최대한 안전하게(NPE) email 추출하기
//		if(order != null) {
