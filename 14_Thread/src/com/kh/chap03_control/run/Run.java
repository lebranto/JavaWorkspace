...
public class Run {
	...
	public void test2() {
		/*
		 * 스레트 컨트롤 메서드
		 * 	- 일지정지(TimeWaiting) 상태인 스레드를 다시 Runnable 상태로 변경하는 메서드
		 * 	1) notify() / notifyAll()
	...
	public void test1() {
		 ...
		 *	1) Thread.sleep(ms)
		 *	 - 지정된 시간동안 스레드를 '일시정지 상태'로 변환하는 메서드
		 *	 - 지정된 시간이 지나면, 다시 실행대기(Runnable)상태로 변경된다. 
		 * 
		 * 	2) wait()
		 * 	 - 동기화된 블럭안에서만 사용되며, 지정된 시간이 지나거나(ms를 썻을때)
		 * 	   다른 스레드가 호출하기 전까지 대기키시는 메서드.
		 * 
		 *  3) join(ms)
		 *   - 지정된 시간동안 다른 스레드를 실행시키고, 현재 스레드는 대기 시키는 메서드
